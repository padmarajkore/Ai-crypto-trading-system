...
class Telegram(RPCHandler):
    ...
    @authorized_only
    async def _ask(self, update: Update, context: CallbackContext) -> None:
        """
        Handler for /ask <question>.
        Sends the question to the AI Agent.
        """
        if not context.args:
            await self._send_msg("❓ Usage: /ask <question>")
            return
            
        question = " ".join(context.args)
        await self._send_msg(f"🤖 Analyzing: '{question}'... Please wait.")
        
        # Define the agent URL
        AGENT_URL = "http://localhost:8000/trigger"
        
        def call_agent_sync():
            try:
                msg = f"User Question via Telegram: {question}. Please answer directly and concisely."
                response = requests.post(AGENT_URL, json={"message": msg}, timeout=600)
                if response.status_code == 200:
                    return "✅ Agent has received your request and is processing it."
                else:
        ...
        try:
             ...
             result = await loop.run_in_executor(None, call_agent_sync)
