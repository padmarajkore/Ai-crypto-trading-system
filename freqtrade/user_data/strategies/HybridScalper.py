...
class HybridScalper(IStrategy):
    """
    Hybrid Scalper Strategy - Combines aggressive scalping with momentum chasing for high-frequency trades.
    Uses ultra-sensitive RSI thresholds, EMA crossovers, and MACD for confirmation.
    """
    ...
    minimal_roi = {
        ...
        "2": 0.008,   # 0.8% after 2 minutes
        "4": 0.004    # 0.4% after 4 minutes
    }

    # Optimal stoploss designed for the strategy.
    # This attribute will be overridden if the config file contains "stoploss".
    stoploss = -0.018  # Slightly tighter stoploss for hybrid approach

    # Trailing stoploss
    trailing_stop = True
    trailing_stop_positive = 0.0025  # Enable trailing stop earlier at 0.25%
    ...
    rsi_sell = IntParameter(55, 75, default=65, space='sell', optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Aggressive long entry conditions
        dataframe.loc[
            (
                # RSI crosses above 35 (oversold to neutral)
                ...
                # Volatility filter - significant price change
                (abs(dataframe['price_change']) > 0.002)
            ),
            "enter_long",
        ] = 1
        ...
        dataframe.loc[
            (
                # RSI crosses below 65 (overbought to neutral)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                # RSI crosses above 35 (take profit)
                (qtpylib.crossed_above(dataframe['rsi'], self.rsi_buy.value)) |
                # EMA crossover in opposite direction
                (qtpylib.crossed_above(dataframe['ema5'], dataframe['ema10'])) |
